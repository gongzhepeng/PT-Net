...
class Attn(nn.Module):
    def __init__(self, channels, sr_ratio, heads):
        nn.Module.__init__(self)
        self.sr = sr_ratio
        if self.sr > 1:
            self.sample = nn.AvgPool2d(kernel_size=1, stride=sr_ratio)
            self.LocalProp = nn.ConvTranspose2d(channels, channels, sr_ratio, stride=sr_ratio, groups=channels)
    ...
    def forward(self, x):  # [32,128,16,16]
        B, C, H, W = x.shape
        if self.sr > 1:
            x = self.sample(x)
            x = x.flatten(2).transpose(1, 2)  # [32,128,256]
        else:
            x = x.flatten(2).transpose(1, 2)

        qkv = self.qkv(x).reshape(B, -1, 3, self.heads, C // self.heads).permute(2, 0, 3, 1, 4)
...
class SelfAttn(nn.Module):
    def __init__(self, channels, sr_ratio, head):
        nn.Module.__init__(self)
        self.norm1 = nn.BatchNorm2d(channels)
        self.attn = Attn(channels, sr_ratio, head)
        ...
        self.pos_embed = nn.Conv2d(channels, channels, kernel_size=3, padding=1, groups=channels)
...
class LGBlock(nn.Module):
    def __init__(self, channels, sr_ratio, head):
        nn.Module.__init__(self)
        self.local = Local(channels)
        self.selfattn = SelfAttn(channels, sr_ratio, head)
...
class Filter(nn.Module):
    def __init__(self, channels):
        nn.Module.__init__(self)
        self.block1 = LGBlock(channels, sr_ratio=4, head=4)
        self.block2 = LGBlock(channels, sr_ratio=2, head=8)
        self.block3 = LGBlock(channels, sr_ratio=1, head=16)
        self.sample1 = PatchMerging(channels)
        self.sample2 = PatchMerging(channels)
        self.fc1 = nn.Conv2d(channels, channels*4, kernel_size=1)
        self.fc2 = nn.Conv2d(channels*2, channels*4, kernel_size=1)
        self.fc4 = nn.Conv2d(channels*2, channels, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x = rearrange(x, 'b h w (p1 p2 c)-> b (h p1) (w p2) c', p1=2, p2=2, c=128).permute(0, 3, 1, 2)  # [32,128,8,8]
        x = self.fc2(torch.cat((x2, x), dim=1)).permute(0, 2, 3, 1)
        x = rearrange(x, 'b h w (p1 p2 c)-> b (h p1) (w p2) c', p1=2, p2=2, c=128).permute(0, 3, 1, 2)  # [32,128,16,16]
        x = self.fc4(torch.cat((x1, x), dim=1))
        return x
